...
@torch.no_grad()
def generate(model, idx, field_size, temperature=1., do_sample=False, top_k=None, isPermutation=False):
    """
    ...
    # offsets = torch.zeros((idx.size(0), field_size+1), device=idx.device)
    # offsets[:, -1] = -float('Inf')
    for _ in range(field_size):
        ...
        logits = logits[:, -1, :] / temperature # + offsets
        ...
        if isPermutation:
            for row, idx_to_remove in enumerate(idx_next):
                offsets[row, idx_to_remove] = -float('Inf')
